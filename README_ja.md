...
**multi-agent-shogun** は、10体のClaude Codeエージェントを同時に実行し、実際の開発組織のように戦略・技術・品質のレイヤーで協調させるシステムです。

**なぜ使うのか？**
- 1つのビジネス課題から、戦略立案・要件定義・実装・テストまでを一気通貫で実行
- 10体の専門家エージェントが、それぞれの役割（CEO, コンサル, エンジニア, 法務...）を遂行
- 階層的な意思決定と品質保証プロセスにより、単体のAIでは不可能な高品質な成果物を生成
- ダッシュボードでリアルタイム進捗確認
...
```
      ユーザー（あなた）
           ...
           ▼ ビジネス課題・命令
    ┌─────────────┐
    │ ORCHESTRATOR│  ← 指揮官（CEO）: 全体統括・意思決定
    └──────┬──────┘
           │
           ├───────────────────────────┐
           ▼ 戦略層                    ▼ 技術・品質層
...
| レイヤー | エージェント | 役割 |
|---------|-------------|------|
| **司令塔** | 👑 Orchestrator | CEO / 指揮官 - あなたの唯一の窓口 |
| **戦略** | 📊 Strategy Consultant | 参謀 - ビジネス分析・市場調査 |
| **企画** | 📋 Product Owner | 要件定義 - 仕様策定・チケット化 |
| **技術統括** | 🔧 Tech Lead | CTO - 技術選定・設計・実装統括 |
| **フロント** | 💻 Frontend Specialist | UI実装 - React/Vue等 |
| **バック** | 🖥️ Backend Specialist | API実装 - Go/Python等 |
| **インフラ** | 🗄️ Database/Infra | 環境構築 - SQL/Terraform/Docker |
| **デザイン** | 🎨 UI/UX Designer | デザイン - ガイドライン策定 |
| **品質** | 🔍 QA Reviewer | テスト - コードレビュー・品質保証 |
| **記録** | 📚 Archivist | 書記 - プロジェクト状態の記録・維持 |

tmuxセッションが作成されます：
- `shogun` (Orchestrator) - ここに接続してコマンドを出す
- `multiagent` (Others) - 9体の専門家がバックグラウンドで稼働

---

## 📖 基本的な使い方

### Step 1: Orchestratorに接続

`shutsujin_departure.sh` 実行後、全エージェントが自動的に指示書を読み込み、待機状態となります。

新しいターミナルを開いてOrchestrator（旧Shogunセッション）に接続：
...
OrchestratorはCEOとして振る舞います。具体的なタスクだけでなく、抽象的な課題も投げられます：

```
新規事業として「高齢者向けの見守りチャットボット」を検討している。
市場規模の試算と、MVPの開発計画を立案せよ。
```

### Step 3: 自動化されたプロセスを見守る

Orchestratorは以下のフローで自律的に動きます：

1. **Strategy Consultant** に市場調査と収益性分析を指示
2. **Product Owner** に要件定義とユーザーストーリー作成を指示
3. あなたに「この計画で進めてよいか」確認
4. **Tech Lead** に実装計画の策定を指示
5. **Frontend/Backend/Designer** に並列実装を指示
6. **QA Reviewer** に品質チェックを指示

Dashboard (`dashboard.md`) または Archivist の記録 (`context/project_state.yaml`) で進捗を確認できます。
...
1. **コンテキストの分離**: 1人のAIに全て任せると、前の指示を忘れたり混乱したりします。役割を細分化することで、各エージェントは「自分の専門領域」に集中し、高いパフォーマンスを維持できます。
2. **相互牽制（Check & Balance）**:
   - Strategy（攻め）とRisk/QA（守り）が対立することで、無謀な計画やバグだらけのリリースを防ぎます。
   - FrontendとBackendがAPI仕様書（Contract）を通じて連携することで、結合時の不整合を減らします。
3. **並列処理**:
   - 設計が決まれば、Frontend、Backend、Infraが同時に作業を開始できます。
4. **人間は「承認」に集中**:
   - コーディングや細かいデバッグはAIに任せ、人間は「要件定義の承認」「デザインの承認」「最終リリースの承認」という重要局面にのみ介入します。
...
5. **競合回避**: 各エージェントに専用ファイルを割り当て

### なぜ dashboard.md は Archivist/Reporting System が更新するのか
